import { IVReelStopAnim } from "../VReel";

class Slide implements IVReelStopAnim {
...
export interface IVReelAnimSequence extends IVReelStopAnim {
    addAnim(anim: IVReelStopAnim): void;
}

class Sequence implements IVReelStopAnim, IVReelAnimSequence {
    declare anims: IVReelStopAnim[];
...
// 搖晃動畫類
class Shake implements IVReelStopAnim {
    declare duration: number;
    declare elapsed: number;
...
export class VReelStopAnimPreset {
    static createSequence(): IVReelAnimSequence {
    ...
    static createSlide(duration: number, distance: number, easing?: (t: number) => number): IVReelStopAnim {
    ...
    static createBounce(duration: number, distance: number, easing?: (t: number) => number): IVReelStopAnim {
        const sequence = VReelStopAnimPreset.createSequence();
        sequence.addAnim(VReelStopAnimPreset.createSlide(duration, distance, easing));
        sequence.addAnim(VReelStopAnimPreset.createSlide(duration, -distance, easing));
        return sequence;
    }
